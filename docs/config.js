var getEntries = function(){
  const ADOPT = [
    {data: ["Java", "JavaScript","TypeScript"]},
    {data: ["Terraform","AWS EC2", "AWS S3", "AWS IaaS" ,"Openstack","Atlassian Suite","Git","Jenkins","Concourse","Ansible","CloudFoundry","Docker","OpenShift", "Kubernetes","AWS PaaS","AWS Elastic Beanstalk"]},
    {data: ["Spring","Spring Cloud","Spring Boot","Angular.js v4+","React.js","React Native"]},
    {data: ["PostgreSQL","MySQL","Oracle","AWS RDS","AWS DynamoDB","MongoDB","Apache Spark","Apache Kafka","AWS Kinesis","AWS EMR","RabbitMQ","AWS SQS","AWS SNS","Elastic Search","Redis"]}
  ]

  const TRIAL = [ {data:[ "Scala","Python","Kotlin"]}, 
                  {data: ["AWS EKS","Google Cloud Platform","OpenStack" ]},
                  {data:["Node.js","Vue.Js","Akka","Play","Scikit-learn","OpenAPI","Flask"]}, 
                  {data: ["AWS Athena","AWS SageMaker","AWS Data Pipeline","Hazelcast","GraphQL","ELK","Neo4J","R","Google BigQuery","Zeppelin"]},
                ]; 

  const ASSESS = [{data: ["Go","Clojure","Haskell","Elm"]}, 
                  {data: [ "Microsoft Azure","Serverless/FaaS","AWS Lambda","AWS EFS"]}, 
                  {data: ["Flutter","Progressive WebApps","Serverless Framework","Bosh","AWS Lex","AWS Translate"]}, 
                  {data: ["AWS Redshift","AWS LakeFormation","AWS Managed Blockchain","AWS Glue","Azure Blockchain Service","Google BigTable","HDFS","Airflow","Google CloudSQL","Google Spanner","Google DataProc","Google DataFlow","Google Tensorflow"]}]

  const HOLD = [{data: ["C","PHP","Ruby",]}, 
                {data: ["AWS ECS","YARN","On premise"]}, 
                {data: ["Backbone.js","Angular.js v1","Apache Wicket"]}, 
                {data: ["OracleDB","IBM DB2","HBase","MySQL"]}]

          // shamelessly loop O^2 
    let dataEntries = []; 
          for(let i =0; i < ADOPT.length; i++){
            let item = ADOPT[i].data; 
         ...
         for(let k =0; k < TRIAL.length; k++){
          ...
          for(let k =0; k < ASSESS.length; k++){
            let item = ASSESS[k].data;
            for(let j =0; j < item.length; j++) {
              let entry = { 
                quadrant: k, 
                ring: 2, 
...
var config = {
    ...
    title: "Comsysto REPLY Techradar — 2019.02",
    quadrants: [
      { name: "Languages" },
      { name: "Infrastructure" },
      { name: "Frameworks" },
